...
- **Launch handshake — do NOT rely on a one-shot phone push (real-hardware trap):** It is tempting to have the phone push the initial data on its JS `ready` event and leave the watch passive. This works *only* in the emulator, whose message delivery is lossless and deterministic. On **real hardware it hangs the watch on its loading screen with no recovery**: the phone's `ready` can fire before the watch's inbox is open, may have *already* fired for that JS session (so it won't fire again for this launch), or the single BT message can simply be dropped — and nothing ever retries. The robust pattern is the reverse: **the watch requests the initial data once its own inbox is open, and retries on an `app_timer` until the first reply arrives** (stop retrying in `inbox_received`). Keep the phone's `ready`-push too as a fast path — belt and suspenders. Have `loadBoard`/data-send be idempotent so both firing is harmless.
- **Watch→phone timing (emulator quirk):** an AppMessage sent from C very early (during `window_load` / right after `app_message_open`) can be dropped by the phone *simulator* (a `QemuInboundPacket.footer` decode exception in the pkjs log); the same send from a **button press works fine**. This is an emulator artifact — **real hardware is more forgiving**, which is exactly why you must not let the emulator talk you out of the watch-driven retry handshake above. If the launch request is dropped in the emulator, the phone's `ready` push still covers it there; the retry covers hardware.
